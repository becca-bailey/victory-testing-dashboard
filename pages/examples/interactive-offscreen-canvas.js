...
//     workerRef.current.postMessage({type: "getNextLineData"});
//   }, []);

//   return (
//     <div>
//       <p>Do work in a WebWorker!</p>
//       <button onClick={handleWork}>Send message</button>
//     </div>
//   );
// };
...
const OffscreenCanvasExample = ({
  ...
  const [nextLineData, setNextLineData] = React.useState();

  const handleMessage = React.useCallback((evt) => {
    setNextLineData(evt.data);
  }, []);

  const getNextLineData = React.useCallback(async () => {
    workerRef.current.postMessage({type: "getNextLineData"});
  }, []);
